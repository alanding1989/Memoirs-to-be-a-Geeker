...
- ##### 进程的定义：  
  一个运行的程序实例就是一个进程。程序本身只是指令、数据及其组织形式的描述。  
...
#### 4. 事件驱动、异步回调和协程
- ##### 为什么会有事件驱动(事件轮询)：  
  服务器领域为了解决单台服务器有限资源处理10K量级的并发连接问题，产生事件驱动编程，如 linux的IO 多路复用
  epoll技术。
...
- ##### 事件驱动和异步回调：  
  ...
  - **优点**： 基于单线程实现，所不需要考虑并发设计和锁，不需要考虑内部调度，并且非阻塞。
...
- ##### 为什么会有协程：  
  ...
  - 虽然有了 Promise 和 Future，但是代码依然有冗余，为了改善异步编程体验，让码农可以用写同步代码的方式编写
    可异步执行的代码，**协程**这个古老的概念又被提出。
...
- ##### 协程和线程的关系：
  - 线程是内核进行抢占式调度的，从而确保每个线程都有执行的机会。  

  - 协程运行在同一个线程中，不需要系统来执行抢占式调度，而是在语言层面实现线程的调度。  

  - 不同**编程语言**协程实现方式及特性有所不同。具体见 [7. 语言相关的特点](#7-语言相关的特点)  
...
- ##### 线程执行的特点：
  - 现代CPU和编译器具有对代码进行指令重排和乱序执行的优化特性。线程调度完全由系统内核控制，多个线程的执行顺
    序无法预测，同一段代码多次运行会产生不同的结果。  
