...
- 引用：  
    >  引用与C++中引用概念差不多，既可以取左值，也可以取暂态变量的右值引用。  
...
- 借用：  
  ...
  有权就会直接转移到函数之中，如果不想进行转移，可获取其引用，称之为借用。

- Rust中函数的三种获取参数的方式及与Cpp对比：

  |      |   获取所有权   |    可变借用    |       不可变借用     |
  | ---  | ---            | ---            | ---                  |
  | Cpp  | && + std::move | & (引用或指针) | const & (引用或指针) |
...
- 获取函数中局部变量，可能是栈上的也可能是堆上动态分配的(操作堆上对象比栈上慢)。
  ...
  - 堆上的：需要堆上动态分配对象的值，若是拷贝，不论是速度或是内存占用都不算最优，可以
    ...
    将该对象地址赋给指针。  
  - **注意:** 绝对不要返回一个局部引用(Rust)或者原始指针(Cpp)！！！
