...
- [2. 传统并发编程的难点](#2-传统并发编程的难点)
  ...
  - [2.2. 如何处理线程之间的竞态条件(线程之间如何通信协调－信息同步)](#22-如何处理线程之间的竞态条件线程之间如何通信协调信息同步)
    ...
    - [屏障：CountDownLatch/CyclicBarrier](#屏障countdownlatchcyclicbarrier)
...
- #### 1.2.3. 利用工具类Executors...造现成的线程池
  注意：无界队列的大坑

- #### 1.2.4. 使用线程池底层API－ThreadPoolExecutor
...
- **synchronized：**  解决了多线程之间数据竞争的问题，但没有解决多线程协调(执行顺序)的问题。  

- `synchronized`关键字和`java.util.concurrent.locks.Lock`都能加锁，两者有什么区别呢？

1. **原始构成**    ：`sync`是JVM层面的，底层通过`monitorenter`和`monitorexit`来实现。`Lock`是JDK API层面的。（`sync`一个enter会有两个exit，一个是正常退出，一个是异常退出）
2. **使用方法**    ：`sync`不需要手动释放锁，而`Lock`需要手动释放。
3. **是否可中断**  ：`sync`不可中断，除非抛出异常或者正常运行完成。`Lock`是可中断的，通过调用`interrupt()`方法。
4. **是否为公平锁**：`sync`只能是非公平锁，而`Lock`既能是公平锁，又能是非公平锁，构造时传入参数。
...
### 屏障：CountDownLatch/CyclicBarrier
> 可以让一系列线程在某个指定的点进行同步。通过让参与指定屏障区域的线程阻塞等待，直到所有线程都到达指定的点。
...
- `CountDownLatch`是减，而`CyclicBarrier`是加，理解了`CountDownLatch`，`CyclicBarrier`就很容易。
  比如召集7颗龙珠才能召唤神龙，详见[CyclicBarrierDemo](./demo-src/thread/CyclicBarrierDemo.java)。
